...
class DrawBrain extends Component {
  ...
  addToDeck = deck => {
    ...
    const userDecktoUpdate=this.state.finalNewCards
    Axios.post("/newDeck", (req, res) => {
   
      User.findOneAndInsert( { username: req.body.username }, {userDeck:userDecktoUpdate }).then(user => {
              console.log(user);
              if (!user) {
